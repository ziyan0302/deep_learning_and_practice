...
class DDPG:
    ...
    def select_action(self, state, noise=True):
        ...
        with torch.no_grad():
            if noise:
                result = self._actor_net(torch.from_numpy(state).view(1, -1).to(self.device)) + \
                    torch.from_numpy(self._action_noise.sample()).view(
                        1, -1).to(self.device)

            else:
                result = self._actor_net(torch.from_numpy(
                    state).view(1, -1).to(self.device))
            # print('result: ', result)
