...
class VAE(nn.Module):  # conditional VAE
    ...
    def forward(self,input_tensor,encoder_hidden_state,encoder_cell_state,c,use_teacher_forcing):
        ...
        print('input_tensor: ', self.dataTransformer.tensor2string(input_tensor))
        ...
        for di in range(input_length):
            ...
            predict_class=output.topk(1)[1] # index of the largest value # output.topk(1)[0] = the largest value
            # print('predict_class: ', self.dataTransformer.tensor2string(predict_class))
            # print('predict_output_becat: ', predict_output)
            predict_output=torch.cat((predict_output,predict_class)) if predict_output is not None else predict_class
            # print('predicted output: ', self.dataTransformer.tensor2string(predict_output))    
        ...
        print('predict_output_final: ', self.dataTransformer.tensor2string(predict_output))
