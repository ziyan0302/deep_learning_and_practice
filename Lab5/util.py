...
def get_kl_weight(epoch,epochs,kl_annealing_type,time):
    ...
    :param time:
        ...
        if('cycle'):     # of cycle
    """
    assert kl_annealing_type=='monotonic' or kl_annealing_type=='cycle' or kl_annealing_type=='monotonic0.1'\
        ,'kl_annealing_type not exist!'

    if kl_annealing_type == 'monotonic':
        return (1./(time-1))*(epoch-1) if epoch<time else 1.

    elif kl_annealing_type=='cycle': #cycle
        period = epochs//time
        epoch %= period
        KL_weight = sigmoid((epoch - period // 2) / (period // 10)) / 2
        return KL_weight
    elif kl_annealing_type=='monotonic0.1':
...
def get_gaussian_score(words):
    ...
    words_list = []
    score = 0
    yourpath = os.path.join('lab5_dataset','train.txt')  #should be your directory of train.txt
