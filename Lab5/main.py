...
if __name__=='__main__':
    ...
    vae.load_state_dict(torch.load(os.path.join('models','best_cycle_time2_epochs500_ref.pt')))
    ...
    for epoch in range(1,epochs+1):
        ...
        print(f'epoch{epoch:>2d}/{epochs}  tf_ratio:{teacher_forcing_ratio:.2f}  kl_weight:{kl_weight:.5f}')
        ...
        print('best BLEU: ', best_BLEUscore)
        ...
        if epoch %100 == 1:
            fig=plot(epoch,CEloss_list,KLloss_list,BLEUscore_list,teacher_forcing_ratio_list,kl_weight_list)
            fig.savefig(os.path.join('results',f'{kl_annealing_type}_time{time}_epochs{epoch}.png'))
