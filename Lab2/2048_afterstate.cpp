...
public:
	...
	// virtual float estimate_all(const board& b ) const {
	// 	float value = 0;
    //     board b0 = b;
    //     int zero_num = b0.empty_num();
    //     std::vector<board> all_board1;
    //     std::vector<board> all_board2;
    //     // info << "before popup"<< b0 << std::endl;
    //     for (int i = 0;i < zero_num; i++){
    //         board b1 = b0;
    //         board b2 = b0;
    //         b1.pop1(i);
    //         b2.pop2(i);
    //         all_board1.push_back(b1);
    //         all_board2.push_back(b2);
    //     }
    //     // info << "all_board1[1]" << all_board1[1] << std::endl;

    //     for (int j = 0;j < zero_num; j++){
    //         value += estimate(all_board1[j]);
    //         // value += operator[](index2)*0.1/zero_num + operator[](index1)*0.9/zero_num;
    //     }
    //     info << "value= " << value << std::endl;
	// 	return value;
	// }
...
public:
	 ...
	 */
	state select_best_move1(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
                ...
                float expected_v = 0;
                float zero_num = move->after_state().empty_num();
                board b0 = move->after_state();
                // info << "b0= " << b0 << std::endl;
                // info << "zero_num= " << zero_num << std::endl;
                
                for (int i = 0;i < zero_num; i++){
                    board b1 = move->after_state();
                    board b2 = move->after_state();
                    b1.pop1(i);
                    b2.pop2(i);
                    // info << "b1= " << b1 << std::endl;
                    // info << "b2= " << b2 << std::endl;
                    expected_v = expected_v + (0.9*1.0/zero_num*estimate(b1) + 0.1*1.0/zero_num*estimate(b2));
                    // info << "add estimation of all_board[" << i << "] "  << expected_v << std::endl;
                }
                
				move->set_value(move->reward() + expected_v);
                // std::cout  << "estimate(move->after_state())= " << estimate(move->after_state()) << std::endl;
                // info << "expected v = " << expected_v << std::endl;
                // std::cout << estimate(move->after_state()) << std::endl;
				if (move->value() > best->value())
					best = move;
                    // info << "best= " << best << std::endl;
			} else {
	...
	state select_best_move0(const board& b) const {
		state after[4] = { 0, 1, 2, 3 }; // up, right, down, left
		state* best = after;
		...
		for (state* move = after; move != after + 4; move++) {
            int i = 0;
			if (move->assign(b)) {
                info << "b0: " << move->after_state() << std::endl;
                info << "b0: " << estimate(move->after_state()) << std::endl;
				move->set_value(move->reward() + estimate(move->after_state()));
                std::cout << "estimate of after_state" << estimate(move->after_state()) << std::endl;
	 ...
	 */
	void update_episode1(std::vector<state>& path,std::vector<board>& board_p,float alpha = 0.1) const {
		float exact = 0;
		for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
                board_p.pop_back();
                board& before_board = board_p.back();
                state& move = path.back();
                float error = exact - estimate(before_board) + move.reward();
                // info << "before" << move.before_state() << std::endl;
                // info << "after" << move.after_state() << std::endl;
                debug << "update error = " << error << " for after state" << std::endl << move.after_state();
                // info<<move.before_state()<<std::endl;
                // exact = move.reward() + update(move.before_state(), alpha * error);
                exact = update(before_board, alpha * error);
...
int main(int argc, const char* argv[]) {
    ...
    tdl.add_feature(new pattern({ 0, 4, 5}));
    ...
    tdl.add_feature(new pattern({8, 9, 10, 11, 12, 13}));
    tdl.add_feature(new pattern({4, 5, 10, 11}));




	// restore the model from file
	tdl.load("/home/arg/courses/machine_learning/homework/Lab2/state100000_0.05.bin");

	// train the model
	std::vector<state> path;
    std::vector<board> board_p;
	path.reserve(20000);
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
            ...
            board_p.push_back(b);
			state best = tdl.select_best_move1(b);
            path.push_back(best);
		...
		// update by TD(0)
		tdl.update_episode1( path ,board_p,alpha);
		tdl.make_statistic(n, b, score);
		path.clear();
	}
    // file.close();
	// store the model into file
	// tdl.save("/home/arg/courses/machine_learning/homework/Lab2/state.bin");
